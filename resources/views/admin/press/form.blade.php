...
@section('page-title', $item->exists ? 'Edit Post' : 'New Post')

@section('content')
<div class="container py-4">
  <h1 class="h4 mb-3">{{ $item->exists ? 'Edit Post' : 'New Post' }}</h1>
  <form method="post" action="{{ $item->exists ? route('admin.press.update', $item) : route('admin.press.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="row g-3">
      ...
      <div class="col-12 mt-3">
        ...
        <a href="{{ route('admin.press.index') }}" class="btn btn-link">Cancel</a>
      </div>
    </div>
  </form>
  </div>
